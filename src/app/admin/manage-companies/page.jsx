...
export default function ManageCompanies() {
  ...
  const handleAddCompany = async () => {
    ...
    try {
      ...
      fetchCompanies();
      document.getElementById("add-company-modal").close();
      setNewCompany({});
      setImage(null);
    } catch (error) {
  ...
  const handleBlockCompany = async (companyId, block) => {
    setLoading(true);
    try {
      await axios.post(
  ...
  const uploadImage = (folderName, imageName, path) => {
    if (!image) {
        ...
        <fieldset className="fieldset w-full">
          ...
          <div className="flex flex-row gap-4">
            ...
            <button
              ...
              onClick={() =>
                document.getElementById("add-company-modal").showModal()
              }
            >
...
const Card = ({ company, handleBlockCompany, handleDeleteCompany }) => {
  return (
